...
// Priority slot pricing (3rd=100, 2nd=120, 1st=150) — displayed in modal
...
const PurchaseModal: React.FC<PurchaseModalProps> = ({
  ...
  const needsJob = item.itemType === "MOCK_INTERVIEW"; // Priority slots are now job-agnostic vouchers

  // For MOCK_INTERVIEW only — job ID must be entered so questions are tailored
  ...
  const isReady = !needsJob || jobId.trim() !== "";

  const handleConfirm = () => {
    const jId = needsJob && jobId.trim() ? Number(jobId.trim()) : null;
  ...
  return (
    <div
      ...
      style={{
        // Inline positioning beats CSS variables — no cascade issues.
        // Matches --layout-navbar-height (58–60px) and BottomDock (64px).
        ...
        <div className="store-modal__body">
          ...
          {isPrioritySlot && (
            <div className="store-modal__rank-section">
              <div className="store-modal__rank-label">SELECT PRIORITY RANK</div>
          ...
          {/* Job association input */}
          {needsJob && (
            <div className="store-modal__job-section">
              <label className="store-modal__job-label">
                {isPrioritySlot ? "APPLY TO JOB (Job ID)" : "JOB ID FOR INTERVIEW"}
              </label>
              <input
                className="store-modal__job-input"
                type="number"
              ...
              <p className="store-modal__job-hint">
                {isPrioritySlot
                  ? "This priority slot will be applied to that job's application queue."
                  : "Interview questions will be tailored to this job's requirements."}
              </p>
            </div>
          )}
...
const StorePage: React.FC = () => {
  ...
  const handleConfirmPurchase = async (associatedJobId: number | null, cost: number, slotRank: number | null) => {
  ...
  const handleOpen = (item: StoreItemWithMeta, purchase: UserPurchaseResponse) => {
    if (item.itemType === "READINESS_REPORT") {
      navigate(`/store/readiness-report/${purchase.id}`);
    } else if (item.itemType === "MOCK_INTERVIEW") {
      navigate(`/store/mock-interview/${purchase.id}`);
    }
  };
...
const styles = `
  .store-page { max-width: 1120px; position: relative; }
  ...
  .store-eyebrow__text { font-family: var(--font-mono); font-size: 10px; font-weight: 700; letter-spacing: 0.22em; color: #FCD34D; text-shadow: 0 0 14px #FCD34D66; }
  .store-header__title { font-family: var(--font-display); font-size: clamp(var(--text-3xl), 4vw, 2.75rem); font-weight: 700; color: var(--color-text-primary); letter-spacing: -0.03em; margin: 0 0 var(--space-2); line-height: 1; }
  ...
  .store-balance-card__label { font-family: var(--font-mono); font-size: 9px; font-weight: 700; letter-spacing: 0.22em; color: #FCD34D55; margin-bottom: var(--space-1); }
  ...
  .store-balance-card__unit { font-family: var(--font-mono); font-size: var(--text-xs); color: #FCD34D88; letter-spacing: 0.15em; margin-top: 2px; }
  .store-balance-card__earn-link { background: none; border: none; font-family: var(--font-mono); font-size: 10px; color: #FCD34D55; cursor: pointer; margin-top: var(--space-2); transition: color var(--duration-fast); letter-spacing: 0.05em; }
  ...
  .store-error-banner__dismiss { margin-left: auto; background: none; border: none; color: #FCA5A5; cursor: pointer; font-size: var(--text-xs); opacity: 0.7; }
  ...
  .store-purchases-banner__link { background: none; border: none; font-family: var(--font-mono); font-size: var(--text-xs); color: var(--color-primary-400); cursor: pointer; letter-spacing: 0.05em; padding: 0; transition: opacity var(--duration-fast); }
  .store-purchases-banner__link:hover { opacity: 0.7; }

  /* Filters */
  .store-filters { display: flex; gap: var(--space-2); flex-wrap: wrap; margin-bottom: var(--space-7); }
  .store-filter-pill { font-family: var(--font-mono); font-size: var(--text-xs); font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; padding: var(--space-2) var(--space-4); border-radius: var(--radius-full); border: 1px solid var(--color-border-default); background: var(--color-bg-elevated); color: var(--color-text-muted); cursor: pointer; transition: all var(--duration-fast); }
  ...
  .store-card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 1px; background: linear-gradient(90deg, transparent, var(--accent, #ffffff33), transparent); opacity: 0.6; }
  ...
  .store-card--unaffordable { opacity: 0.65; }
  ...
  .store-card__badge { position: absolute; top: 14px; right: 14px; font-family: var(--font-mono); font-size: 9px; font-weight: 700; letter-spacing: 0.15em; padding: 3px 8px; border-radius: var(--radius-full); z-index: 2; }

  .store-card__content { display: flex; flex-direction: column; gap: var(--space-4); flex: 1; }
  .store-card__icon-wrap { width: 56px; height: 56px; border-radius: var(--radius-xl); display: flex; align-items: center; justify-content: center; border: 1px solid rgba(255,255,255,0.08); flex-shrink: 0; }
  .store-card__icon { font-size: 1.75rem; }

  .store-card__info { display: flex; flex-direction: column; gap: var(--space-2); }
  .store-card__name { font-family: var(--font-display); font-size: var(--text-xl); font-weight: 700; color: var(--color-text-primary); margin: 0; letter-spacing: -0.02em; }
  .store-card__tagline { font-family: var(--font-mono); font-size: var(--text-xs); font-weight: 600; letter-spacing: 0.06em; margin: 0; }
  ...
  .store-card__price-unit { font-family: var(--font-mono); font-size: var(--text-xs); color: var(--color-text-muted); letter-spacing: 0.1em; }
  .store-card__owned-label { font-family: var(--font-mono); font-size: var(--text-sm); font-weight: 600; color: #34D399; letter-spacing: 0.05em; }

  .store-card__cta { padding: var(--space-2) var(--space-5); border-radius: var(--radius-lg); font-family: var(--font-mono); font-size: var(--text-xs); font-weight: 700; letter-spacing: 0.06em; cursor: pointer; transition: all var(--duration-fast); white-space: nowrap; border: none; }
  ...
  .store-card__cta--buy:disabled { opacity: 0.4; cursor: not-allowed; }
  ...
  /* ── Purchase modal ────────────────────────────── */
  .store-modal-backdrop { position: fixed; left: 0; right: 0; background: rgba(0,0,0,0.75); backdrop-filter: blur(6px); display: flex; align-items: center; justify-content: center; z-index: 1000; padding: var(--space-5); animation: fade-in 0.15s ease-out; }
  @keyframes fade-in { from { opacity: 0; } to { opacity: 1; } }

  .store-modal { width: 100%; max-width: 520px; max-height: 100%; overflow-y: auto; background: var(--color-bg-surface); border: 1px solid var(--color-border-strong); border-radius: var(--radius-2xl); box-shadow: 0 24px 80px #000000bb; animation: modal-slide-up 0.2s cubic-bezier(0.34, 1.56, 0.64, 1); }
  @keyframes modal-slide-up { from { transform: translateY(20px); opacity: 0; } to { transform: translateY(0); opacity: 1; } }

  .store-modal__header { display: flex; align-items: center; gap: var(--space-4); padding: var(--space-6) var(--space-6) var(--space-5); }
  ...
  .store-modal__tagline { font-family: var(--font-mono); font-size: var(--text-xs); letter-spacing: 0.06em; margin: 0; }
  .store-modal__close { background: none; border: none; color: var(--color-text-muted); cursor: pointer; font-size: var(--text-base); padding: var(--space-2); border-radius: var(--radius-md); transition: all var(--duration-fast); flex-shrink: 0; }
  .store-modal__close:hover { background: var(--color-bg-overlay); color: var(--color-text-primary); }

  .store-modal__body { padding: var(--space-5) var(--space-6); display: flex; flex-direction: column; gap: var(--space-5); }
  ...
  .store-modal__rank-label { font-family: var(--font-mono); font-size: 10px; font-weight: 700; letter-spacing: 0.15em; color: var(--color-text-muted); }
  .store-modal__rank-options { display: flex; gap: var(--space-3); }
  .store-rank-option { flex: 1; display: flex; flex-direction: column; align-items: center; gap: var(--space-1); padding: var(--space-3) var(--space-2); border-radius: var(--radius-lg); border: 1px solid var(--color-border-default); background: var(--color-bg-overlay); cursor: pointer; transition: all var(--duration-fast); }
  .store-rank-option:hover { border-color: var(--color-border-strong); }
  .store-rank-option--selected { background: var(--color-bg-elevated); }
  ...
  /* Job input */
  .store-modal__job-section { display: flex; flex-direction: column; gap: var(--space-2); }
  .store-modal__job-label { font-family: var(--font-mono); font-size: 10px; font-weight: 700; letter-spacing: 0.15em; color: var(--color-text-muted); }
  .store-modal__job-input { width: 100%; padding: var(--space-3) var(--space-4); background: var(--color-bg-overlay); border: 1px solid var(--color-border-default); border-radius: var(--radius-lg); color: var(--color-text-primary); font-family: var(--font-mono); font-size: var(--text-sm); outline: none; transition: border-color var(--duration-fast); box-sizing: border-box; }
  .store-modal__job-input:focus { border-color: var(--color-primary-400); }
  .store-modal__job-hint { font-size: var(--text-xs); color: var(--color-text-muted); margin: 0; line-height: 1.5; }
  ...
  .store-modal__balance-label { font-family: var(--font-mono); font-size: 10px; letter-spacing: 0.1em; color: var(--color-text-muted); }
  ...
  .store-modal__footer { display: flex; gap: var(--space-3); justify-content: flex-end; padding: var(--space-4) var(--space-6) var(--space-6); border-top: 1px solid var(--color-border-subtle); }
  ...
  .store-btn-purchase { padding: var(--space-3) var(--space-6); border-radius: var(--radius-lg); font-family: var(--font-mono); font-size: var(--text-sm); font-weight: 700; letter-spacing: 0.05em; border: none; cursor: pointer; color: white; min-width: 160px; transition: all var(--duration-fast); }
  ...
  .store-spinner { display: inline-block; width: 14px; height: 14px; border: 2px solid rgba(255,255,255,0.3); border-top-color: white; border-radius: 50%; animation: spin 0.6s linear infinite; }
  ...
  @keyframes drain-anim { 0% { transform: translate(-50%, -50%) scale(0.8); opacity: 0; } 20% { transform: translate(-50%, -60%) scale(1.2); opacity: 1; } 80% { transform: translate(-50%, -90%) scale(1); opacity: 1; } 100% { transform: translate(-50%, -110%) scale(0.9); opacity: 0; } }
  .xp-drain-text { font-family: var(--font-display); font-size: 2.5rem; font-weight: 700; color: #FCD34D; text-shadow: 0 0 30px #FCD34D88; }

  /* Responsive */
  @media (max-width: 900px) { .store-header { flex-direction: column; gap: var(--space-5); } .store-grid { grid-template-columns: repeat(2, 1fr); } }
  @media (max-width: 640px) { .store-grid { grid-template-columns: 1fr; } .store-modal { border-radius: var(--radius-2xl) var(--radius-2xl) 0 0; } .store-modal__rank-options { flex-direction: column; } }
`;
